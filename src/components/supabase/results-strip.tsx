  ...
  return (
    <section>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-2xl font-bold text-slate-950">Results and fixtures</h2>
        <span className="text-sm text-slate-500">From Supabase</span>
      </div>
      {matches.length ? (
          ...
          {matches.map((match) => (
            <article key={match.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-slate-950">{scoreLabel(match)}</span>
              </div>
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500">
          No matches in Supabase yet. Run the football-data sync.
        </div>
      )}
    </section>
  );
...
function TeamLine({ team }: { team: Team | null }) {
  return (
    <div className="mt-3 flex items-center gap-3">
      <span className="grid size-9 place-items-center overflow-hidden rounded-full border border-slate-200 bg-slate-50">
        {team?.crest_url ? (
      ...
      <span>
        <span className="block text-sm font-semibold text-slate-950">{teamLabel(team)}</span>
